...
static const uint8_t expected_seed_s[SABER_NOISE_SEEDBYTES] = {
    0x86, 0x26, 0xed, 0x79, 0xd4, 0x51, 0x14, 0x08, 0x00, 0xe0, 0x3b, 0x59, 0xb9, 0x56, 0xf8, 0x21,
    0x0e, 0x55, 0x60, 0x67, 0x40, 0x7d, 0x13, 0xdc, 0x90, 0xfa, 0x9e, 0x8b, 0x87, 0x2b, 0xfb, 0x8f};

static const uint8_t expected_m_raw[SABER_KEYBYTES] = {0xc8, 0x2c, 0xe0, 0x50, 0xa6, 0xdd, 0x85, 0xfe, 0xa6, 0x3d, 0xd0,
                                                       0x65, 0x6a, 0xf1, 0x46, 0xb1, 0x88, 0x0f, 0x91, 0xab, 0xc0, 0x07,
                                                       0x2c, 0x92, 0xa9, 0xda, 0x17, 0x78, 0x76, 0x9c, 0x46, 0x61};

static const char expected_pkey_hex[] = "29157c69a067dd1c151115a9bbeb2d6e627f4e747716efa1569c7b7858cb2e71"
...
static const char expected_sk_hex[] = "ff3f000480000000feff000800fe5f00088000f0ff018000f0ffff5f00fcffff"
...
static const char expected_ct_hex[] = "719fac31ae90417f2da1d37e47a065b860575d1df2de60814a39297b902afa2f"
...
static int check_round_trip(void) {
    ...
    if (decode_hex(expected_pkey_hex, expected_pkey, sizeof(expected_pkey)) != 0) {
        printf("[FAIL] check_pke_round_trip: expected pkey decode failed\n");
        return 1;
    }

    if (decode_hex(expected_sk_hex, expected_sk, sizeof(expected_sk)) != 0) {
        printf("[FAIL] check_pke_round_trip: expected sk decode failed\n");
        return 1;
    }

    if (decode_hex(expected_ct_hex, expected_ct, sizeof(expected_ct)) != 0) {
        printf("[FAIL] check_pke_round_trip: expected ct decode failed\n");
        return 1;
    }
    ...
    if (memcmp(pkey, expected_pkey, sizeof(pkey)) != 0) {
        printf("[FAIL] check_pke_round_trip: pkey mismatch\n");
        return 1;
    }

    if (memcmp(sk.sk, expected_sk, sizeof(sk.sk)) != 0) {
        printf("[FAIL] check_pke_round_trip: sk mismatch\n");
        return 1;
    }

    if (memcmp(ct.bytes, expected_ct, sizeof(ct.bytes)) != 0) {
        printf("[FAIL] check_pke_round_trip: ct mismatch\n");
        return 1;
    }

    if (memcmp(dec, m, SABER_KEYBYTES) != 0) {
        printf("[FAIL] check_pke_round_trip: round-trip decryption failed\n");
        return 1;
    }

    printf("[PASS] check_pke_round_trip\n");
    return 0;
}
