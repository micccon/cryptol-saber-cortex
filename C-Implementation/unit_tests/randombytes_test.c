...
/* Print buffers in the same compact hex form used by the Python helper script. */
...
int main(void) {
    /* Fixed KAT seed from the HighLevelSpec DRBG expansion script. */
    ...
    /* These are the first four 32-byte requests made by the DRBG for kat0. */
    static const uint8_t expected_seed_a[32] = {
    ...
    static const uint8_t expected_seed_s[32] = {
    ...
    /* Mirror the deterministic expansion used by the KAT-generation script. */
    ...
    /* Check each request independently so failures point to the first divergence. */
