...
static const uint8_t expected_ss[SABER_KEYBYTES] = {0x15, 0x65, 0x33, 0x53, 0x6c, 0x84, 0x35, 0xf8, 0x2c, 0xc3, 0x6f,
                                                    0xc1, 0xef, 0x95, 0x28, 0xde, 0xdc, 0x49, 0x22, 0x3d, 0xda, 0x00,
                                                    0x91, 0x61, 0x7d, 0xc1, 0xac, 0xaf, 0x60, 0x58, 0xd1, 0xca};
...
static int check_round_trip(void) {
    ...
    if (decode_hex(expected_pkey_hex, expected_pkey, sizeof(expected_pkey)) != 0) {
        printf("[FAIL] check_kem_round_trip: expected pkey decode failed\n");
        return 1;
    }

    if (decode_hex(expected_ct_hex, expected_ct, sizeof(expected_ct)) != 0) {
        printf("[FAIL] check_kem_round_trip: expected ct decode failed\n");
        return 1;
    }
    ...
    if (memcmp(pkey, expected_pkey, sizeof(pkey)) != 0) {
        printf("[FAIL] check_kem_round_trip: pkey mismatch\n");
        return 1;
    }

    if (memcmp(ct.bytes, expected_ct, sizeof(ct.bytes)) != 0) {
        printf("[FAIL] check_kem_round_trip: ct mismatch\n");
        return 1;
    }

    if (memcmp(ss_enc, expected_ss, SABER_KEYBYTES) != 0) {
        printf("[FAIL] check_kem_round_trip: shared secret mismatch\n");
        return 1;
    }

    if (memcmp(ss_enc, ss_dec, SABER_KEYBYTES) != 0) {
        printf("[FAIL] check_kem_round_trip: round-trip decapsulation failed\n");
        return 1;
    }

    printf("[PASS] check_kem_round_trip\n");
    return 0;
}
